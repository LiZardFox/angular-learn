...
import { AmbientLight, BoxGeometry, Color, DodecahedronGeometry, Group, Mesh, MeshBasicMaterial, MeshStandardMaterial, PlaneGeometry, TorusGeometry } from "three";
...
@Component({
    ...
    template: `
        ...
        @for (scene of scenes; track scene.name) {
            <ngt-mesh [position]="[(viewport().width + slideDistance) * $index, 0, 0]">
    ...
    imports: [NgtArgs, NgtsEnvironment, NgtsGrid, NgtsRenderTexture, RenderTextureScene, CameraHandler],
})
export class Scene {
    ...
    constructor() {
        extend({ Mesh, BoxGeometry, MeshStandardMaterial, MeshBasicMaterial, Color, AmbientLight, Group, TorusGeometry, DodecahedronGeometry, PlaneGeometry });
    }
}